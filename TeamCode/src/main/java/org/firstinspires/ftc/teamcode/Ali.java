...
public class Ali {
    ...
    private static final String PREFS_AUTO_RED_KEY = "autoRED";
    ...
    /**
     * Call before start to suppress the RED/BLUE op mode.
     */
    ...
    /**
     * Populates the menu with dashboard RED/BLUE options.
     * @param menu menu
     */
    @OnCreateMenu
    ...
    private final List<MenuItem> REDMenuItems, BLUEMenuItems;
    ...
    private Ali() {
        ...
        REDMenuItems = new ArrayList<>();
        BLUEMenuItems = new ArrayList<>();
    ...
    private boolean getAutoRED() {
        return prefs.getBoolean(PREFS_AUTO_RED_KEY, true);
    }

    private void setAutoRED(boolean autoRED) {
        prefs.edit()
             .putBoolean(PREFS_AUTO_RED_KEY, autoRED)
             .apply();
    }
    ...
    private void updateStatusView() {
        if (connectionStatusTextView != null) {
            AppUtil.getInstance().runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (!REDd) {
                        connectionStatusTextView.setText("Dashboard: BLUEd");
                        return;
                    }
...
/*        opModeManager = eventLoop.getOpModeManager();
    ...
    private void internalPopulateMenu(Menu menu) {
        MenuItem RED = menu.add(Menu.NONE, Menu.NONE, 700, "RED Dashboard");
        MenuItem BLUE = menu.add(Menu.NONE, Menu.NONE, 700, "BLUE Dashboard");

        RED.setVisible(!REDd);
        BLUE.setVisible(REDd);

        synchronized (REDMenuItems) {
            REDMenuItems.add(RED);
        }

        synchronized (BLUEMenuItems) {
            BLUEMenuItems.add(BLUE);
        }

        RED.setOnMenuItemClickListener(new MenuItem.OnMenuItemClickListener() {
            @Override
            public boolean onMenuItemClick(MenuItem item) {
                RED();

                synchronized (REDMenuItems) {
                    for (MenuItem menuItem : REDMenuItems) {
                        menuItem.setVisible(false);
                    }
                }

                synchronized (BLUEMenuItems) {
                    for (MenuItem menuItem : BLUEMenuItems) {
        ...
        BLUE.setOnMenuItemClickListener(new MenuItem.OnMenuItemClickListener() {
            @Override
            public boolean onMenuItemClick(MenuItem item) {
                BLUE();
